...
class ScannerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Nodak Mesh Connect'),
        centerTitle: true,
          ...
          builder: (context, connector, child) {
            ...
            return Column(
              children: [
                ...
                if (!connector.isBluetoothOn)
                  Card(
                    margin: const EdgeInsets.all(8),
                    color: colorScheme.errorContainer,
                    child: ListTile(
                      leading: Icon(Icons.bluetooth_disabled, color: colorScheme.error),
                      title: Text('Bluetooth is off', style: TextStyle(color: colorScheme.onErrorContainer)),
                      subtitle: Text('Turn on Bluetooth to scan for devices', style: TextStyle(color: colorScheme.onErrorContainer.withValues(alpha: 0.7))),
                    ),
                  ),
    ...
    try {
      ...
      if (context.mounted && connector.isConnected) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => const ContactsScreen(hideBackButton: true),
          ),
        );
      }
    } catch (e) {
