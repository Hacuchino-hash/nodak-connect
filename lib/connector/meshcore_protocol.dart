...
// Build CMD_SEND_TRACE_PATH frame
// Format: [cmd][tag x4][auth_code x4][flags][path...]
// tag: unique identifier for correlating request/response (usually timestamp)
// auth_code: authorization code (can be 0 for basic trace)
// flags: trace flags (0 for standard trace)
// path: array of 1-byte repeater hashes (first byte of pub key)
Uint8List buildSendTracePathFrame(
  Uint8List path, {
  int? tag,
  int authCode = 0,
  int flags = 0,
}) {
  final traceTag = tag ?? (DateTime.now().millisecondsSinceEpoch ~/ 1000);
  final frame = Uint8List(1 + 4 + 4 + 1 + path.length);
  int offset = 0;

  frame[offset++] = cmdSendTracePath;
  writeUint32LE(frame, offset, traceTag);
  offset += 4;
  writeUint32LE(frame, offset, authCode);
  offset += 4;
  frame[offset++] = flags;
  frame.setRange(offset, offset + path.length, path);

  return frame;
}

/// Parsed trace data response from PUSH_CODE_TRACE_DATA (0x89)
class TracePathResult {
  final int tag;
  final int authCode;
  final int flags;
  final List<int> pathHashes;
  final List<double> pathSnrs; // SNR in dB (already divided by 4)
  final int hopCount;
  ...
  const TracePathResult({
    ...
    required this.pathSnrs,
    required this.hopCount,
  });

  @override
  String toString() {
    final snrStr = pathSnrs.map((s) => '${s.toStringAsFixed(1)}dB').join(', ');
    return 'TracePathResult(hops: $hopCount, snrs: [$snrStr])';
  }
}

/// Parse PUSH_CODE_TRACE_DATA frame
/// Format: [code][reserved][path_len][flags][tag x4][auth_code x4][path_hashes...][path_snrs...]
TracePathResult? parseTraceDataFrame(Uint8List frame) {
  if (frame.isEmpty || frame[0] != pushCodeTraceData) return null;
  if (frame.length < 12) return null; // Minimum: code + reserved + path_len + flags + tag + auth_code

  final pathLen = frame[2];
  final flags = frame[3];
  final tag = readUint32LE(frame, 4);
  final authCode = readUint32LE(frame, 8);

  // Path hashes start at offset 12
  final hashesStart = 12;
  final snrsStart = hashesStart + pathLen;

  // Verify frame has enough data
  if (frame.length < snrsStart + pathLen) {
    // Frame might be truncated, use what we have
    final availableHashes = frame.length > hashesStart
        ? (frame.length - hashesStart).clamp(0, pathLen)
        : 0;

    final pathHashes = <int>[];
    final pathSnrs = <double>[];

    for (int i = 0; i < availableHashes; i++) {
      pathHashes.add(frame[hashesStart + i]);
    }
  ...
  for (int i = 0; i < pathLen; i++) {
    pathHashes.add(frame[hashesStart + i]);
    // SNR is stored as SNR * 4, divide to get actual dB value
    final rawSnr = frame[snrsStart + i];
    // Handle signed SNR values (can be negative)
    final signedSnr = rawSnr > 127 ? rawSnr - 256 : rawSnr;
    pathSnrs.add(signedSnr / 4.0);
  }
