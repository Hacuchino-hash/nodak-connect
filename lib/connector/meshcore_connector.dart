...
class MeshCoreConnector extends ChangeNotifier {
  ...
  final StreamController<TracePathResult> _traceResultsController =
      StreamController<TracePathResult>.broadcast();
  ...
  final Map<int, bool> _channelMuted = {};
  ...
  Stream<TracePathResult> get traceResults => _traceResultsController.stream;
  ...
  bool get isBluetoothOn => _bluetoothState == BluetoothAdapterState.on;
  ...
  bool isChannelMuted(int channelIndex) {
    return _channelMuted[channelIndex] ?? false;
  }

  Future<void> setChannelMuted(int channelIndex, bool muted) async {
    _channelMuted[channelIndex] = muted;
    await _channelSettingsStore.saveMuted(channelIndex, muted);
    notifyListeners();
  }
  ...
  Future<void> loadChannelSettings({int? maxChannels}) async {
    ...
    for (int i = 0; i < channelCount; i++) {
      ...
      _channelMuted[i] = await _channelSettingsStore.loadMuted(i);
    }
  }
  ...
  Future<void> startScan({Duration timeout = const Duration(seconds: 10)}) async {
    ...
    if (!isBluetoothOn) {
      debugPrint('[Scan] Bluetooth is off');
      return;
    }
  ...
  /// Send a trace path command to measure SNR through the specified path.
  /// Returns a unique tag that will be echoed in the response.
  /// Listen to [traceResults] stream to receive the TracePathResult.
  Future<int> sendTracePath(Uint8List path, {int authCode = 0}) async {
    final tag = DateTime.now().millisecondsSinceEpoch ~/ 1000;
    if (!isConnected) {
      debugPrint('[Trace] Cannot send trace - not connected');
      return tag;
    }
  ...
  Future<void> getChannels({int? maxChannels, bool forceRefresh = false}) async {
    if (!isConnected) return;
    ...
    if (_channelsLoaded && !forceRefresh && maxChannels == null) {
      debugPrint('[ChannelSync] Channels already loaded, skipping (use forceRefresh to reload)');
      return;
    }
  ...
  /// Updates last message time for a contact matched by name or path.
  /// Returns true if the sender was found in contacts.
  bool _updateContactLastMessageAtByName(
    String senderName,
    DateTime timestamp, {
    Uint8List? pathBytes,
    bool notify = false,
  ...
  void _maybeNotifyChannelMessage(
    ...
    // Skip notification if channel is muted
    if (isChannelMuted(channelIndex)) return;
  ...
  // Track recently notified unknown nodes to avoid spam
  final Set<String> _recentlyNotifiedNodes = {};

  void _maybeNotifyNodeDiscovered(String nodeName) {
    ...
    // Normalize name and check if we recently notified for this node
    final normalized = nodeName.trim().toLowerCase();
    if (normalized.isEmpty || normalized == 'unknown') return;
    if (_recentlyNotifiedNodes.contains(normalized)) return;

    // Track this node and auto-clear after 5 minutes
    _recentlyNotifiedNodes.add(normalized);
    Future.delayed(const Duration(minutes: 5), () {
      _recentlyNotifiedNodes.remove(normalized);
    });

    _notificationService.showAdvertNotification(
      contactName: nodeName,
      contactType: 'Node',
    );
  }
  ...
  void _handleIncomingChannelMessage(Uint8List frame) {
    ...
    if (message != null && message.channelIndex != null) {
      ...
      final senderInContacts = _updateContactLastMessageAtByName(
        message.senderName,
        message.timestamp,
      ...
      if (isNew) {
        ...
        // Notify for unknown node discovery
        if (!senderInContacts && message.senderName.isNotEmpty) {
          _maybeNotifyNodeDiscovered(message.senderName);
        }
      }
      _handleQueuedMessageReceived();
    } else if (_isSyncingQueuedMessages) {
